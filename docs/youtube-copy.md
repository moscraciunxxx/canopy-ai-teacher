# YouTube publication copy

Published at https://youtu.be/VyyJqFhyBb4

## Title

Canopy — One AI Teacher Across 9 Subjects and 20 Languages | 2-Minute Demo

## Description

Canopy is an interactive, inspectable AI learning academy built for the SPEED
August AI Challenge. It combines nine subject-native visual laboratories, a
course-specific reasoning graph, adaptive teaching modes, and a complete
20-language interface—including chart legends, hover labels, diagrams,
feedback, and sources.

In this two-minute walkthrough:

- manipulate a browser-rendered 3D physics trajectory;
- rotate and edit a DNA helix;
- compare historical perspectives and civic evidence flows;
- explore ethical trade-offs without outsourcing the verdict;
- move from Coach to Learn, Remix, and Apply/Roleplay;
- switch languages without losing the learner's course state.

Source code: https://github.com/moscraciunxxx/canopy-ai-teacher

Try the account-free live app:
https://canopy-ai-teacher.moscraciuncv.chatgpt.site

No Streamlit account, sign-in, API key, or high-end GPU is required.

Built with Python, Streamlit, Plotly, HTML/CSS/SVG, Unicode BCP 47, Docker,
pytest, Ruff, and mypy.

#AI #EdTech #MachineLearning #Streamlit #Devpost #Education
